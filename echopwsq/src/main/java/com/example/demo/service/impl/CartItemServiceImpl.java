...
public class CartItemServiceImpl implements CartItemService {
    ...
    public void saveCartItems(List<CartItem> cartItems) {
        for (CartItem item : cartItems) {
            if (item.getCart() == null || item.getProduct() == null) {
...
//            existingCart.setCart_price(updatedCart.getCart_price());
    ...
    public void deleteCartItem(Long cartItemId) {
        ...
        if (item != null) {
            ...
            if (cart != null && cart.getItems() != null) {
                cart.getItems().remove(item); // 移除集合
            }
            CartItemDAO.delete(cartItemId); // 刪除資料庫
        }
	...
	public List<CartItem> findByCart(Cart cart) {
		// 從資料庫撈出這個 Cart 的所有 CartItem
		return CartItemDAO.findByCart(cart);
	}

	@Override
    public void saveCartItem(CartItem cartItem) {
		CartItemDAO.save(cartItem); // ✅ Spring Data JPA 自動 insert/update
    }


	@Override
	public void saveCartItem1(CartItem cartItem) {
		// TODO Auto-generated method stub
		
	}

	@Override
	public void saveCartItem(Object cartItem) {
		// TODO Auto-generated method stub
